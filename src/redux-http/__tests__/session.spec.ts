import { beccaccinoSession } from '../session';
...
describe('beccaccinoSessionGenerator', () => {
  const sessionManager = beccaccinoSession();
  ...
  it('tests sessionManager structure with stringify', () => {
    expect(JSON.stringify(sessionManager)).toBe(JSON.stringify(mockSessionManager));
  });
  it('tests sessionManager get function', () => {
    expect(new Date(sessionManager.get()).getMilliseconds()).toBeGreaterThanOrEqual(
  ...
  it('tests sessionManager refresh function with mock', async () => {
    const oldSessionManagerTime = new Date(sessionManager.get()).getMilliseconds();
    await sleep(2000);
    sessionManager.refresh();
    const newSessionManagerTime = new Date(sessionManager.get()).getMilliseconds();
